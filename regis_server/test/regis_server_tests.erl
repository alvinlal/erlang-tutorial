...
% 6. A process should only be registered by a string for name
% 7. An unregistered name should return 'not_found' on process lookup
% 8. A process can be unregistered and should no longer appear on lookup
% 9. Unregistering a process with a name that does not exists should not cause an error
% 10. A process that was registered can be registered again if it was unregistered between both calls
% 11. A crashed process should be unregistered
...
register_test_() ->
    ...
    {"A process should only be registered by a string for name",
    ?setup(fun register_string/1)},
...
register_string(_) ->
   Name = "test_process",
   spawn(fun() -> testProcess(Name) end),
   Res = regis_server:register(atomname,self()),
   [?_assertEqual({error,bad_type},Res)].
